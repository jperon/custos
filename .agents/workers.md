...
- **In :** NFQUEUE `QUEUE_RESPONSES` ; pipe `question_response` (lecture via `drain_pipe`) ; pipe ACK `nft` dédié ; fd d'écriture `events` (`events_wfd`, injecté par `main.moon`) ; `sessions.lua` (cache mtime).
...
- **Retry upstream (`dns.upstream_retry`) :** sur une réponse transitoirement en échec (rcode ∈ `rcodes`, défaut SERVFAIL/REFUSED), ne transmet pas la réponse au client : réémet la même question vers **le même résolveur** (`build_query_from_response` reconstruit la requête QR=0 ; `dup_query.build_query` inverse src/dst + ports ; `raw_send` via socket RAW, src client spoofée) et `NF_DROP` la réponse en échec. La transaction reste **non consommée** (`entry.upstream_retries` incrémenté, `entry.expire` prolongé) : la réponse du retry repasse par le pont, est recapturée et appariée à la même `entry`. Au-delà de `max_attempts`, la réponse en échec est transmise normalement. Évite le « refusé puis OK au rafraîchissement » dû à un upstream instable (ex. dynv6) : sans RR A/AAAA, aucune IP n'entre dans l'allowlist nft. `try_upstream_retry` / `arm_retry_fds` (sockets RAW par famille armés dans `run`). **NXDOMAIN** (rcode 3) est retenté par défaut (l'autoritatif dynv6 renvoie NXDOMAIN par intermittence pour des noms qui existent, sur tous les résolveurs) ; pour ne pas pénaliser les NXDOMAIN légitimes, un nom dont **même le retry** reste NXDOMAIN entre dans un cache `nxdomain_bad` (`lib.ttl_set`, TTL `nxdomain_bad_ttl`) et n'y est plus retenté jusqu'à ce qu'il résolve de nouveau (NOERROR le réhabilite).
- **Second avis (action `validate`) :** identifie la réponse **validateur** (src ∈ `resolvers`) et l'utilise pour le verdict (`dns_classify` : NXDOMAIN→block ; **sinkhole `0.0.0.0`/`::`→sinkhole** — DNSforFamily bloque par `NOERROR + A 0.0.0.0`, pas par NXDOMAIN ; CNAME→redirect ; sinon pass) sans jamais la transmettre (NF_DROP). N'entre en jeu que si `entry.modifiers.validate` est vrai (règle portant l'action `validate`). Corrélation `(client, txid, qname)` (`second_opinion.moon`). La réponse d'origine est **parquée** (verdict NFQUEUE différé, retour `-1` sans poser de verdict) jusqu'à l'arrivée du verdict ou l'expiration de `budget_ms` (balayage via le timeout `poll` de `nfq_loop` → `sweep_parked`, fail-open). Application via `finalize_a` : `build_nxdomain_response` (block), `build_sinkhole_response` (reproduit les adresses nulles, sinkhole) ou `build_cname_response` + injection nft (redirect). L'EDE porte « Filtered by upstream validator » (pas la raison d'autorisation locale). Les overrides **block** et **sinkhole** émettent en plus une ligne d'événement `block` (`format_block_event` → `events_wfd`) pour que ces blocages apparaissent dans les events agrégés et la liste `/refusals` du portail — sinon ils resteraient invisibles, `worker_questions` ayant déjà loggé la requête en `allow`.
