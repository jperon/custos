...
- **Second avis (`second_opinion.enabled`) :** pour chaque question UDP autorisée, **duplique** le paquet (dst réécrite vers une IP `resolvers` de la bonne famille via `dup_query`, src/txid/qname préservés) et l'émet via un **socket RAW routé par le noyau** (`raw_send`, `IP_HDRINCL`/`IPV6_HDRINCL`, src = IP client) : pas de MAC de passerelle à connaître, IPv6 sur tunnel géré nativement. Une famille n'est activée que si un validateur de cette famille est routable. Best-effort, ne bloque pas.
...
- **Second avis (`second_opinion.enabled`) :** identifie la réponse **validateur** (src ∈ `resolvers`) et l'utilise pour le verdict (`dns_classify` : NXDOMAIN→block ; **sinkhole `0.0.0.0`/`::`→sinkhole** — DNSforFamily bloque par `NOERROR + A 0.0.0.0`, pas par NXDOMAIN ; CNAME→redirect ; sinon pass) sans jamais la transmettre (NF_DROP). Corrélation `(client, txid, qname)` (`second_opinion.moon`). La réponse d'origine est **parquée** (verdict NFQUEUE différé, retour `-1` sans poser de verdict) jusqu'à l'arrivée du verdict ou l'expiration de `budget_ms` (balayage via le timeout `poll` de `nfq_loop` → `sweep_parked`, fail-open). Application via `finalize_a` : `build_nxdomain_response` (block), `build_sinkhole_response` (reproduit les adresses nulles, sinkhole) ou `build_cname_response` + injection nft (redirect). L'EDE porte « Filtered by upstream validator » (pas la raison d'autorisation locale).
