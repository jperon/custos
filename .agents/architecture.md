...
| `QUEUE_SNI` | `worker_tls` | `tcp/udp dport 443 queue to N bypass` (opt. ; **avant** `cv_action_vmap` si `placement=integral`, **après** si `residual`) | `NF_ACCEPT`/`NF_DROP` selon `auth.sni_verdict` | Paire client→dest dans `ip*_allowed`/`mac*_allowed` via pipe `nft` |
