...
nftables (table bridge)
    ...
    │
    ├─ Port 33443     → QUEUE_AUTH      → worker_auth_queue → auth_ipc pipe (22 B) → auth/worker
    │
...
| `worker_auth_queue` | NFQUEUE sur `QUEUE_AUTH` ; extrait MAC/IP, écrit dans `auth_ipc` |
...
| `auth/worker` | Portail HTTPS captif (luasec), lit `auth_ipc` |
...
| `learn` pipe | 22 octets : ip16 + mac6 | `worker_questions` + `worker_arp_sniffer` → `mac_learner` |
| `auth_ipc` pipe | 22 octets : ip16 + mac6 | `worker_auth_queue` → `auth/worker` |
...
| `QUEUE_AUTH` | `worker_auth_queue` | trafic port 33443 | `NF_ACCEPT` | Aucune ; mac+ip dans auth_ipc |
