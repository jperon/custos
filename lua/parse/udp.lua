...
local pseudo_header_sum_v6
pseudo_header_sum_v6 = function(src_ip_raw, dst_ip_raw, udp_len)
  local sum = 0
  for i = 1, 15, 2 do
    sum = sum + read_u16(src_ip_raw, i)
  end
  for i = 1, 15, 2 do
    sum = sum + read_u16(dst_ip_raw, i)
  end
  sum = sum + udp_len
  sum = sum + 17
  return sum
...
checksum_udp = function(raw, ip_hdr, udp_hdr)
  local sum
  if ip_hdr.version == 6 then
    sum = pseudo_header_sum_v6(ip_hdr.src_ip_raw, ip_hdr.dst_ip_raw, udp_hdr.udp_len)
  else
    sum = pseudo_header_sum_v4(ip_hdr.src_ip_raw, ip_hdr.dst_ip_raw, udp_hdr.udp_len)
  end
...
return {
  ...
  pseudo_header_sum_v6 = pseudo_header_sum_v6,
