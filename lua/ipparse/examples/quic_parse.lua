...
local crypto_frame_len_hex = string.format("%04x", bor(0x4000, crypto_frame_data_len))
...
xor_hex_strings = function(hex_a, hex_b)
  ...
  for i = 1, min_len do
    local a = string.byte(bin_a, i)
    local b = string.byte(bin_b, i)
    result_bin = result_bin .. string.char(band(bor(a, b), bnot(band(a, b))))
  end
