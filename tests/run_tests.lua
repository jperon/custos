...
local build_nxdomain = m_dns.build_nxdomain
...
test("build_nxdomain -- header NXDOMAIN + synthetic A + EDE OPT", function()
  ...
  local nxdomain = build_nxdomain(dns_obj, dns_buf)
  assert(nxdomain, "build_nxdomain nil")
  local resp = parse_dns(nxdomain)
  ...
  return assert_eq(resp.answers[1].rdata_str, "0.0.0.0", "synthetic A = 0.0.0.0")
end)
test("build_nxdomain -- header NXDOMAIN + synthetic AAAA + EDE OPT", function()
  local qname = "\8facebook\3com\0"
  local dns_buf = make_dns(qname, QTYPE.AAAA, false, 0xDEAD)
  local dns_obj = parse_dns(dns_buf)
  assert(dns_obj, "parse_dns nil")
  local nxdomain = build_nxdomain(dns_obj, dns_buf)
  assert(nxdomain, "build_nxdomain nil")
  ...
  assert_eq(resp.hdr.ancount, 1, "ancount=1 (synthetic answer)")
  assert_eq(#resp.answers, 1, "1 synthetic answer")
  return assert_eq(resp.answers[1].rdata_str, "0:0:0:0:0:0:0:0", "synthetic AAAA = :: (expanded)")
end)
