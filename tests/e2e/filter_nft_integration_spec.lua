...
assert_contains = function(str, substr, msg)
  if not (str:find(substr, 1, true)) then
    return error(tostring(msg or 'assert_contains failed') .. ": '" .. tostring(substr) .. "' not found")
  end
end
...
local compiled_rules = rule.compile_rules(filter_cfg)
...
local plan = nft_compiler.compile(filter_cfg, compiled_rules.rules_metadata)
...
return print("- Runtime decision simulation: OK")
