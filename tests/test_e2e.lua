...
run_check("scp -O " .. tostring(SSH_OPTS) .. " -i " .. tostring(SSH_KEY) .. " libvirt/config.moon " .. tostring(FILTER_USER) .. "@" .. tostring(FILTER_IP) .. ":/etc/custos/config.moon")
...
test("curl https://tracker.test → reject <500ms", function()
  local start = os.time()
  local ok, out = ssh_client(FILTER_IP, "curl -k -s -o /dev/null -w '%{http_code}' --max-time 3 " .. "https://tracker.test/ 2>&1; echo exit=$?")
  local elapsed = os.time() - start
  assert(elapsed < 3, "trop lent (" .. tostring(elapsed) .. "s) — reject n'a pas RST ?")
