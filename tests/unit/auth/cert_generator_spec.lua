...
return describe("auth/cert_generator", function()
  ...
  it("sans=nil et jours=nil → valeurs par défaut, px5g absent → erreur exit code", function()
    local cert, key, ok, err = generate_self_signed("example.com")
    assert.is_false(ok)
    assert.is_not_nil(err)
    return assert.is_string(err)
  end)
  it("CN valide avec jours explicites → chemin px5g (px5g absent → erreur)", function()
    local cert, key, ok, err = generate_self_signed("test.local", { }, 365)
    assert.is_false(ok)
    assert.is_not_nil(err)
    return assert.is_string(err)
  end)
