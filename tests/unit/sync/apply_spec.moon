...
-- Charge un fichier Lua produit par apply (return { ... }).
load_result = (path) ->
...
after_each ->
  ...
  os.execute "rm -f #{TMP}/out.lua #{TMP}/device-explicit.moon #{TMP}/custom-out.lua"
...
describe "sync/apply — comportement nominal", ->
  ...
  it "base seule si aucun device ne correspond (hostname inconnu)", ->
    ...
    ret, _ = run "--base #{TMP}/base/config.moon --hostname nobody --output #{TMP}/out.lua"
    assert.equals 0, ret
    cfg = assert load_result "#{TMP}/out.lua"
  ...
  it "les valeurs device écrasent la base (merge superficiel)", ->
    ...
    ret, _ = run "--base #{TMP}/base/config.moon --hostname router1 --output #{TMP}/out.lua"
    assert.equals 0, ret
    cfg = assert load_result "#{TMP}/out.lua"
  ...
  it "--device spécifie explicitement le chemin de la config device", ->
    ...
    ret, _ = run "--base #{TMP}/base/config.moon --device #{TMP}/device-explicit.moon --output #{TMP}/out.lua"
    assert.equals 0, ret
    cfg = assert load_result "#{TMP}/out.lua"
  ...
  it "--output spécifie un chemin de sortie personnalisé", ->
    write_file "#{TMP}/base/config.moon", [[ { runtime: { log_level: "INFO" } } ]]
    ret, _ = run "--base #{TMP}/base/config.moon --hostname nobody --output #{TMP}/custom-out.lua"
    assert.equals 0, ret
    assert.truthy read_file "#{TMP}/custom-out.lua"

  it "le fichier de sortie est un Lua valide rechargeable par loadfile", ->
    write_file "#{TMP}/base/config.moon", [[{
    ...
    ret, _ = run "--base #{TMP}/base/config.moon --hostname nobody --output #{TMP}/out.lua"
    assert.equals 0, ret
    cfg, err = load_result "#{TMP}/out.lua"
    assert.is_nil err
    assert.not_nil cfg
    assert.equals "INFO",  cfg.runtime.log_level
  ...
  it "affiche un message de confirmation sur la sortie standard", ->
    write_file "#{TMP}/base/config.moon", [[ { runtime: { log_level: "INFO" } } ]]
    ret, out = run "--base #{TMP}/base/config.moon --hostname nobody --output #{TMP}/out.lua"
    assert.equals 0, ret
  ...
  it "--reload ne plante pas si le service est inactif (SIGHUP ignoré)", ->
    ...
    ret, _ = run "--base #{TMP}/base/config.moon --hostname nobody --output #{TMP}/out.lua --reload"
    assert.equals 0, ret
    assert.truthy read_file "#{TMP}/out.lua"
