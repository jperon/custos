-- tests/test_ndpi.lua
-- Unit tests for the pure FFI nDPI-based parser (parse/ndpi).
-- Requires libndpi installed (apt install libndpi-dev).
...
local function make_dns(qname_encoded, qtype, is_response, txid, ancount)
  txid = txid or 0x1234
  qtype = qtype or 1
...
test("ffi_ndpi — version detected from ndpi_revision()", function()
  ...
  -- We know at least 4.x is installed.
...
test("parse_packet — DNS question A www.github.com", function()
  ...
  -- nDPI detection: master_protocol = DNS (5), app_protocol varies by domain
...
test("parse_answers — CNAME record", function()
  ...
  local rr = "\xC0\x0C"
    .. string.char(0, 5, 0, 1)       -- type CNAME, class IN
    .. string.char(0, 0, 0, 60)      -- TTL = 60
    .. string.char(0, #cname_data)   -- rdlength
    .. cname_data
...
test("patch_and_checksum — TTL rewritten to 60", function()
  ...
  -- Re-parse to verify TTL changed.
