-- src/filter/actions/strip_A.moon
-- Action : enlever les enregistrements A de la réponse DNS.
...
(require "ipc").register_modifier "strip_a"
  ...
  (rule) ->
    {
      ...
      eval: (req) ->
        true, "Strip A by rule: #{rule.description or '?'}"
      on_response: (ctx) ->
        stripped = strip_dns_rr ctx.dns_raw, "A"
