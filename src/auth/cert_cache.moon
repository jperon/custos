...
create_cache = (max_size = 500, ttl = 7776000, cert_dir = "tmp/certs") ->
  ...
  save_cert_to_disk = (hostname, cert_pem, key_pem) ->
    ...
    unless cert_fh
      log_warn { action: "cert_cache_disk_write_failed", file: cert_file }
      return false
    ...
    unless key_fh
      log_warn { action: "cert_cache_disk_write_failed", file: key_file }
      os.remove cert_file
      return false
  ...
  set = (hostname, cert_pem, key_pem, ctx) ->
    ...
    unless save_cert_to_disk hostname_lower, cert_pem, key_pem
      log_warn { action: "cert_cache_set_disk_failed", hostname: hostname_lower }
      return false
