...
extract_sni = (data) ->
  unless data and #data >= 43
    log_debug { action: "sni_extract_too_short", len: #data or 0 }
    return nil
  
  -- Vérifier type record TLS = Handshake (0x16)
  record_type = read_u8 data, 1
  unless record_type == 0x16
    log_debug { action: "sni_extract_not_handshake", type: record_type }
    return nil
  ...
  unless #data >= 5 + record_length
    log_debug { action: "sni_extract_truncated_record", avail: #data, need: 5 + record_length }
    return nil
  ...
  unless hs_type == 0x01
    log_debug { action: "sni_extract_not_clienthello", hs_type: hs_type }
    return nil
  ...
  unless cipher_suites_offset + 1 <= #data
    log_debug { action: "sni_extract_truncated_cipher_suites" }
    return nil
  ...
  unless compression_offset + 1 <= #data
    log_debug { action: "sni_extract_truncated_compression" }
    return nil
  ...
  unless extensions_offset + 1 <= #data
    log_debug { action: "sni_extract_no_extensions" }
    return nil
  
  extensions_len = read_u16_be data, extensions_offset
  unless extensions_len > 0
    log_debug { action: "sni_extract_empty_extensions" }
    return nil
  ...
  unless ext_data_end <= #data
    log_debug { action: "sni_extract_truncated_extensions" }
    return nil
  ...
  while pos < ext_data_end
    ...
    if ext_type == 0x0000
      ...
      unless ext_payload_offset + 1 <= #data
        log_debug { action: "sni_extract_snl_truncated" }
        return nil
      ...
      unless snl_offset + 2 <= #data
        log_debug { action: "sni_extract_sn_header_truncated" }
        return nil
      ...
      unless name_type == 0
        log_debug { action: "sni_extract_unknown_name_type", type: name_type }
        return nil
      
      name_offset = snl_offset + 3
      unless name_offset + name_len - 1 <= #data
        log_debug { action: "sni_extract_sn_name_truncated", need: name_offset + name_len, have: #data }
        return nil
      ...
      unless hostname\match "^[a-zA-Z0-9._*-]+$"
        log_warn { action: "sni_extract_invalid_hostname", hostname: hostname }
        return nil
      
      log_debug { action: "sni_extract_found", hostname: hostname }
      return hostname
    
    -- Passer à l'extension suivante
    pos = ext_payload_offset + ext_len
  
  log_debug { action: "sni_extract_no_sni_extension" }
  nil
