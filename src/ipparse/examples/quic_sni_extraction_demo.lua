...
demonstrate_sni_extraction = function(quic_packets)
  ...
  if #successful_frames > 0 then
    ...
    sni_ext_data = string.char(0x00, 0x00, (ext_len >> 8) & 0xFF, ext_len & 0xFF, (list_len >> 8) & 0xFF, list_len & 0xFF, 0x00, (name_len >> 8) & 0xFF, name_len & 0xFF) .. mock_sni
    local extensions_data = string.char((ext_len + 4) >> 8 & 0xFF, (ext_len + 4) & 0xFF) .. sni_ext_data
    client_hello_data = client_hello_data .. extensions_data
    local hs_len = #client_hello_data - 9
    client_hello_data = client_hello_data:sub(1, 6) .. string.char((hs_len >> 16) & 0xFF, (hs_len >> 8) & 0xFF, hs_len & 0xFF) .. client_hello_data:sub(10)
    local record_len = #client_hello_data - 5
    client_hello_data = client_hello_data:sub(1, 3) .. string.char((record_len >> 8) & 0xFF, record_len & 0xFF) .. client_hello_data:sub(6)
